...
function(req, res) {
    ...
    if (req.isAuthenticated()) {
        ...
        // 로그인이 실패한 경우에는 이미 failureRedirect에 의해 응답이 보내진 상태
        // 따라서 여기서는 다시 응답을 보내지 않도록 주의
        console.log('로그인 실패');
    }
...
passport.use(new facebookStrategy({
    clientID: '1154717232161518',
    clientSecret: '26150caced33947b7683d5f464694c8d',
...
function(accessToken, refreshToken, profile, done){
    ...
    while(loop < 2)
    {
        mydb.collection('account').findOne({userkey : authkey}).then((result) => {
            if (result != null){
                done(null, result);
            } else {
                mydb.collection('account').insertOne({userkey : authkey, userid : authname,})
            }
        }).catch((error) => {
